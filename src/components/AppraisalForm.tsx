...
export const AppraisalForm: React.FC<AppraisalFormProps> = ({
  ...
  useEffect(() => {
    if (appraisal) {
      setFormData({
        ...
        inspection_date_ro:
          (appraisal as any).inspection_date_ro ||
        ...
        doc_building_license_no:
          (appraisal as any).doc_building_license_no ||
          inspection?.building_license_no ||
          "",
        doc_building_license_date:
          (appraisal as any).doc_building_license_date || "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const appraisalData = {
        ...
        market_value: numOrNull(formData.market_value),
        land_value: numOrNull(formData.land_value),
        building_value: numOrNull(formData.building_value),
        valuation_method: formData.valuation_method,
        final_value: numOrNull(formData.final_value),
        ...
        inspection_date_ro:
          formData.inspection_date_ro || inspection?.inspection_date || null,
        ...
        doc_building_license_no:
          formData.doc_building_license_no ||
          inspection?.building_license_no ||
          null,
        ...
        public_services: Array.isArray(formData.public_services)
          ? formData.public_services
          : [],
        health_services: Array.isArray(formData.health_services)
      ...
      const completedAt =
        formData.status === "completed"
          ? appraisal?.completed_at ?? new Date().toISOString()
          : null;
      (appraisalData as any).completed_at = completedAt;
  ...
  const handleMarkCompleted = async () => {
    ...
    try {
      ...
      setError(e?.message || "تعذر إنهاء التقييم");
    } finally {
  ...
  if (!canEdit && appraisal) {
    return (
      <div className="space-y-6">
        <div
          className="bg-gradient-to-br from-amber-50 to-yellow-50 p-6 rounded-xl border-2 border-amber-300"
          dir="rtl"
        >
          ...
          <p className="text-4xl font-bold text-amber-700">
            {appraisal.final_value?.toLocaleString("ar-SA")} ريال
          </p>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4 text-sm text-gray-700">
            <div>
              قيمة السوق:{" "}
              <span className="font-medium">
                {appraisal.market_value?.toLocaleString("ar-SA")} ريال
              </span>
            </div>
            <div>
              قيمة الأرض:{" "}
              <span className="font-medium">
                {appraisal.land_value?.toLocaleString("ar-SA")} ريال
              </span>
            </div>
            <div>
              قيمة المبنى:{" "}
              <span className="font-medium">
                {appraisal.building_value?.toLocaleString("ar-SA")} ريال
              </span>
            </div>
            <div>
              طريقة التقييم:{" "}
              <span className="font-medium">{appraisal.valuation_method}</span>
            </div>
            <div>
              مستوى الثقة:{" "}
              <span className="font-medium">{appraisal.confidence_level}</span>
            </div>
          </div>
          {appraisal.notes && (
            <div className="mt-4">
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6" dir="rtl">
      {profile?.role && profile.role !== "client" && (
        <div className="mb-4">
          <ClientInfoCard property={property} />
        </div>
      )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setFormData({ ...formData, land_value: e.target.value })
              }
              className="w-full px-4 py-3 border rounded-lg"
            />
          </div>
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setFormData({ ...formData, building_value: e.target.value })
              }
              className="w-full px-4 py-3 border rounded-lg"
            />
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setFormData({ ...formData, valuation_method: e.target.value })
              }
              className="w-full px-4 py-3 border rounded-lg"
            >
      ...
      <div className="bg-white rounded-xl border p-4 space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <select
              value={formData.value_basis}
              onChange={(e) =>
                setFormData({ ...formData, value_basis: e.target.value })
              }
              className="w-full px-4 py-3 border rounded-lg"
            >
              <option value="">اختر</option>
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setFormData({ ...formData, method_used: e.target.value })
              }
              className="w-full px-4 py-3 border rounded-lg"
            >
              <option value="">اختر</option>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">
              تاريخ المعاينة (من تقرير المعاينة)
            </label>
            <input
              type="date"
              value={
                formData.inspection_date_ro || inspection?.inspection_date || ""
              }
              readOnly
          ...
          <div>
            ...
            <input
              type="time"
              value={formData.inspection_time_ro}
              onChange={(e) =>
                setFormData({ ...formData, inspection_time_ro: e.target.value })
              }
              className="w-full px-4 py-3 border rounded-lg"
            />
          </div>
      ...
      <div className="bg-white rounded-xl border p-4 space-y-4">
        <h3 className="font-semibold">الحدود والخدمات</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">
              شمالاً بطول
            ...
            <input
              ...
              onChange={(e) =>
                setFormData({ ...formData, boundary_north: e.target.value })
              }
              className="w-full px-4 py-3 border rounded-lg"
            />
          </div>
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setFormData({ ...formData, boundary_south: e.target.value })
              }
              className="w-full px-4 py-3 border rounded-lg"
            />
          </div>
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setFormData({ ...formData, boundary_east: e.target.value })
              }
              className="w-full px-4 py-3 border rounded-lg"
            />
          </div>
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setFormData({ ...formData, boundary_west: e.target.value })
              }
              className="w-full px-4 py-3 border rounded-lg"
            />
          </div>
          <div className="md:col-span-1">
            <label className="block text-sm font-medium text-gray-700 mb-2">
              الخدمات والمراكز الحكومية (سطر لكل خدمة)
            </label>
            <textarea
              rows={4}
              value={
                Array.isArray(formData.public_services)
                  ? formData.public_services.join("\n")
                  : formData.public_services
                  ? String(formData.public_services)
                  : ""
              }
              onChange={(e) =>
                setFormData({
                  ...formData,
                  public_services: e.target.value
                    .split("\n")
                    .map((s) => s.trim())
          ...
          <div className="md:col-span-1">
            <label className="block text-sm font-medium text-gray-700 mb-2">
              الخدمات الصحية والطبية (سطر لكل خدمة)
            </label>
            <textarea
              rows={4}
              value={
                Array.isArray(formData.health_services)
                  ? formData.health_services.join("\n")
                  : formData.health_services
                  ? String(formData.health_services)
                  : ""
              }
              onChange={(e) =>
                setFormData({
                  ...formData,
                  health_services: e.target.value
                    .split("\n")
                    .map((s) => s.trim())
                    .filter(Boolean) as any,
                })
      ...
      <div className="flex items-center justify-end gap-3">
        {" "}
        {canEdit && appraisal && appraisal.status !== "completed" && (
          <button
            type="button"
          ...
          >
            {completeLoading ? "جارٍ الحفظ..." : "انهاء التقييم"}
          </button>
        )}
        <button
          type="submit"
          disabled={loading}
          className="bg-amber-600 text-white px-5 py-3 rounded-lg hover:bg-amber-700 disabled:opacity-50"
        >
